...
file1 <- "data/globi/all_13858.tsv.gz"

catg <- read.csv(gzfile(categories), sep = "\t", header=FALSE, row.names = NULL)
fN <- data.frame(read.csv(file1, sep = "\t", quote= "", header=TRUE, row.names = NULL)) #no empty lines
...
write.table(fN, file=gzfile("data/globi/intxn.tsv.gz"),sep="\t")
